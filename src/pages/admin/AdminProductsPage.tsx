  ...
  return (
    <div className="admin-page">
      <div className="admin-page-header">
        <h1 className="admin-page-title font-display text-[36px] italic text-[#1A1A1A]">Products</h1>
        <div className="admin-page-actions">
          <Link
            to="/admin/products/new"
            className="rounded-[2px] bg-[#1A1A1A] px-7 py-3 text-center font-body text-[11px] uppercase tracking-[0.1em] text-[#F5F0E8] transition-colors hover:bg-[#C4A882] hover:text-[#1A1A1A]"
          >
            Add New Product
          </Link>
        </div>
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="min-w-[980px] w-full border-collapse">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={7} className="px-0 py-8 text-center font-body text-[12px] text-[#777777]">
                  Loading products...
                </td>
              </tr>
            ) : loadError ? (
                ...
                return (
                  <tr key={product.id} className="border-b border-[#e4dbd1] hover:bg-[rgba(196,168,130,0.04)]">
                    ...
                    <td className="px-2 py-4">
                      ...
                      <p className="mt-0.5 font-body text-[10px] text-[#777777]">{product.sku || "No SKU"}</p>
                    </td>
                    ...
                    <td className="px-2 py-4">
                      ...
                      {product.compare_at_price ? (
                        <p className="font-body text-[11px] text-[#777777] line-through">
                    ...
                    <td className="px-0 py-4">
                      <div className="flex items-center gap-2 font-body text-[10px] uppercase tracking-[0.1em]">
                        <Link to={`/admin/products/${product.id}/edit`} className="text-[#555555] transition-colors hover:text-[#1A1A1A]">
                          Edit
                        </Link>
                        <span className="text-[#d4ccc2]">Â|</span>
                        <button
                          type="button"
                          disabled={deletingId === product.id}
                          onClick={() => void onDelete(product)}
                          className="text-[#555555] transition-colors hover:text-[#C0392B] disabled:cursor-not-allowed disabled:opacity-50"
                        >
      ...
      <div className="border-t border-[#d4ccc2] md:hidden">
        {isLoading ? (
          <p className="py-8 text-center font-body text-[12px] text-[#777777]">Loading products...</p>
        ) : loadError ? (
          <p className="py-8 text-center font-body text-[12px] text-[#C0392B]">{loadError}</p>
        ) : rows.length === 0 ? (
          <p className="py-8 text-center font-body text-[12px] text-[#777777]">No products found.</p>
        ) : (
          rows.map((product) => {
            const threshold = product.low_stock_threshold ?? 5;
            const isLow = product.stock_quantity <= threshold && product.stock_quantity > 0;
            ...
            return (
              <div key={product.id} className="admin-mobile-card">
                <div className="flex gap-3">
                  <div className="h-[53px] w-[40px] overflow-hidden bg-[#ede5db]">
                    {product.image_url ? (
                      <img src={product.image_url} alt={product.name} className="h-full w-full object-cover" />
                    ) : (
                      <div className="h-full w-full bg-[#e2d9cf]" />
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-body text-[12px] text-[#1A1A1A]">{product.name}</p>
                    <p className="mt-1 truncate font-body text-[10px] text-[#777777]">{product.sku || "No SKU"}</p>
                  </div>
                </div>

                <div className="mt-2 flex items-center justify-between gap-3">
                  <p className="font-body text-[10px] uppercase tracking-[0.08em] text-[#C4A882]">
                    {product.category_name || "Uncategorized"}
                  </p>
                  <p className="font-body text-[12px] text-[#1A1A1A]">{formatCurrency(product.price)}</p>
                </div>
                ...
                <div className="mt-2 flex justify-end gap-3 font-body text-[10px] uppercase tracking-[0.1em]">
                  <Link to={`/admin/products/${product.id}/edit`} className="text-[#555555] hover:text-[#1A1A1A]">
                    Edit
                  </Link>
                  <button
                    type="button"
                    disabled={deletingId === product.id}
                    onClick={() => void onDelete(product)}
                    className="text-[#555555] hover:text-[#C0392B] disabled:cursor-not-allowed disabled:opacity-50"
                  >
                    {deletingId === product.id ? "Deleting..." : "Delete"}
                  </button>
                </div>
